#pragma config(I2C_Usage, I2C1, i2cSensors)
#pragma config(Sensor, dgtl1,  button1,        sensorTouch)
#pragma config(Sensor, dgtl2,  button2,        sensorTouch)
#pragma config(Sensor, I2C_1,  ,               sensorQuadEncoderOnI2CPort,    , AutoAssign )
#pragma config(Motor,  port1,           motor1,        tmotorVex393_HBridge, openLoop, encoderPort, I2C_1)
#pragma config(Motor,  port10,           motor2,        tmotorVex393_HBridge, openLoop, encoderPort, I2C_1)
#pragma config(Motor,  port2,           motor3,        tmotorVex393_HBridge, openLoop, encoderPort, I2C_1)
//motor3 hasn't been enabled yet

bool button1_pushed; //flag to store button1 input
bool button2_pushed; //flag to store button2 input

void monitorInput()
{
    if(SensorValue(button1) && !button1_pushed)
    {
        button1_pushed = true;
    }

    if(SensorValue(button2) && !button2_pushed)
    {
        button2_pushed = true;
    }
}
void drive()
{
	      resetMotorEncoder(motor1);
        resetMotorEncoder(motor2);
        while(getMotorEncoder(motor1) <= 1500){
            motor[motor1] = 75; //- getMotorEncoder(motor1)/9;
            motor[motor2] = -75;// - getMotorEncoder(motor1)/9;
        }
        button2_pushed = false;
        button1_pushed = false;

        motor[motor1] = 0;
        motor[motor2] = 0;

} // end exercise_drive

void turn()
{
	    resetMotorEncoder(motor1);
      resetMotorEncoder(motor2);

      while(getMotorEncoder(motor1) <= 700){
                motor[motor1] = 100-getEncoderForMotor(motor1)/9;
                motor[motor2] = 100-getEncoderForMotor(motor1)/9;
      }
      button2_pushed = false;
      button1_pushed = false;//resets buttons

      motor[motor1] = 0;
      motor[motor2] = 0;//stops motors

} // end exercise_turn

task main ()
{
	button1_pushed = button2_pushed = false;
	      resetMotorEncoder(motor1);
        resetMotorEncoder(motor2);

	while(true){
			monitorInput();
		if(button2_pushed){
			turn();
			}
		if(button1_pushed){
			drive();
			}
	}//while
}//main
